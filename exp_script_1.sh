...
watchfile=/homes/cx2/federated/results/exp_script_1.log
...
do
    ...
    do
        ...
        do 
            cat $PBS_NODEFILE | uniq > $PBS_O_WORKDIR/hostfile1
            mpirun -np 10 -machinefile $PBS_O_WORKDIR/hostfile1 python /homes/cx2/federated/fedrob/fedrob.py --classes 10 --model default --nsplit 100 --batchsize 50 --nbyz 0 --lr ${lr} --alpha 1 --alpha-decay ${alphadecay} --alpha-decay-epoch 400 --epochs 800 --iterations 1 --seed ${seed} --dir $inputdir --valdir $valdir --log $logfile 2>&1 | tee $watchfile
        done
    done
done
